...
# とりあえず途中まで形式化
## ゲームの設定部分（パラメータ）
- \(n\): \(\mathbb{N}_{\geq 2}\) ... プレイヤーの人数
- \(X\): \(\text{finite Set}\) ... カードの全体
- \(S\): \(\text{finite Set}\) ... 属性の全体
- \(s_x\): \(\mathcal{P}(S)\) ... 各カード \(x \in X\) に対してその属性を表す部分集合
- \(l, m\): \(\mathbb{N}\) ... 頭に \(l\) 枚、手元に \(m\) 枚配られる枚数
- \(D\): \(X\) の \(\{\text{head} | \text{hand} \} \times \{1 \ldots n\}\) で添え字づけられた分割
  -  \(X = H^{\text{head}}_ 1 \sqcup \cdots \sqcup H^{\text{head}}_ n \sqcup H^{\text{hand}}_ 1 \sqcup \cdots \sqcup H^{\text{hand}}_ n\) であって \(\# H^{\text{head}}_ {*} = l, \# H^{\text{hand}}_ {*} = m\) を満たすもののこと

## 定義部分
- \(M^{\text{Q}}_i\): \(\{(j, s) \in \{1..=n\} \times S \mid i \neq j\}\) ... プレイヤー \(i\) が他のプレイヤー \(j\) に対して属性 \(s \in S\) の見える枚数を聞く質問
- \(A^{\text{Q}}\): \(D \times M^{\text{Q}}_ i \to \mathbb{N}\) ... 回答。
  - \(((H, (j, s))\) に対して、見えているカード全体 \(T = \{H^{\text{hand}}_ i \cup \bigcup_ {k \neq j} H^{\text{head}}_ k)\}\) に対して、属性 \(s\) を持つカードの枚数 \(\# \{x \in T \mid s \in s_x\}\)
- \(M^{\text{D}}_i\): \(\mathcal{P}(X)\) ... プレイヤー \(i\) が頭のカードが \(X' \subset X\) であることを宣言する操作
- \(A^{\text{D}}\): \(D \times M^{\text{D}}_i \to \text{bool} = \{\text{true}, \text{false}\}\) ... 回答。 \((H, X') \mapsto H^{\text{head}}_i == X'\)

ゲームの推移は次のもので記述できる。
- \(H\): \(D\) ... 一番初めの状態
- \(m_1, \ldots, m_k\): 次を満たす有限列 ... 質問と回答の組
...
- \(X\): \(\text{Set}\) ... 状態の全体
- \(x_0\): \(X\) ... 初期状態
- \(P\): \(X \rightarrow \{1..=n\}\) ... 各状態ごとにどのプレイヤーが行動をするか
- \(M\): \(X \rightarrow \mathcal{P}(X)\) ... 各状態からどんな行動ができるか
...
- \(\Lambda\): \(X = \bigsqcup_{\lambda \in \Lambda} X_{\lambda}\) を与える添え字集合 ... プレイヤーからは識別できない状態への分割
- \(\lambda(x)\): \(\lambda \in \Lambda\) s.t. \(x \in X_{\lambda}\) ... 状態 \(x \in X\) に対応する識別ラベル
- \(s \in S \implies \# X_{\lambda(s)} = 1\) ... ゲームが終了する場合は識別が一意にできる条件
- \(X_0\): \(X_0 \subset X\) かつ \(0 \in \Lambda\) ... 可能な初期状態
- \(P\): \(\Lambda \rightarrow \{1 ..= n\}\) ... 各 \(\lambda \in \Lambda\) で行動するプレイヤー
- \(M_\lambda\): 行動の集合 ... 各 \(\lambda \in \Lambda\) でとることのできる行動一覧
    - ただし終了状態からは行動できない... \(s \in S\) なら \(M_{\lambda(s)} = \emptyset\)
- \(M\): \(X_{\lambda} \times M_{\lambda} \rightarrow X\) ... その行動によるゲームの推移
#### 注意点
